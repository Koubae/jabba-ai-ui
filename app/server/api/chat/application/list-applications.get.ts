...
export default defineEventHandler(async (event) => {
    ...
    const endpoint = `${URL}/api/v1/application/list`
    try {
        ...
        logger.info(`List Applications, response: ${response}`)
        return response
    } catch (error) {
        ...
        throw createError({
            ...
            statusMessage: fetchError.statusText || 'Unexpected error while getting Applications'
        })
    }
